...
def main():
    ...
    if int(login_mode) == Login.CONSUMER.value:
        od_engine.login(config.urls['redirect'], config.client['id'], CLIENT_SECRET)
    elif int(login_mode) == Login.BUSINESS.value:
        od_engine.login_business(config.urls['redirect'], config.client['id'], CLIENT_SECRET, config.urls['discovery'], config.urls['auth_server'], config.urls['auth_token'])
